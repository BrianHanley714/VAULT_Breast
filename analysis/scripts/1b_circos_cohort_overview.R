...
clinical_data = clinical_data%>%
  ...
  dplyr::filter(!is.na(prop_tumour_lo))%>%
...
clinical_data_long = bind_rows(clinical_data_long, 
                               clinical_data_long%>%
                                 dplyr::filter(!duplicated(Trial.ID))%>%
...
# Generate the plot -------------------------------------------------------
clinical_data_long%>%
  mutate(bar_outline = if_else(grepl("prop", sample_type), "grey", "white"))%>%
  mutate(label = if_else(duplicated(Trial.ID), "", Trial.ID),
         angle = 270-((360/nrow(clinical_data))*order))%>%
  ggplot() +
  geom_bar(aes(x = as.factor(order), y = Proportion, fill = sample_type, col = bar_outline), stat = "identity", position = "stack", width = 1) +
  ...
  geom_labelhline(yintercept = 1+relsize_tumadj_norm+inner_width, label = "Tumour Adjacent", fontface = "bold", size = 3)+
  geom_labelhline(yintercept = 1+relsize_tumadj_norm*2+inner_width, label = "Distant Normal",fontface = "bold", size = 3)+
  ...
  scale_colour_manual(values = c("white" = "white", "grey" = "grey"))+
  theme(axis.title = element_blank(),
        axis.text = element_blank(),
        legend.position = "none")+
  geom_text(aes(order, radius - out_width +0.2, label = label, angle = angle), fontface = "bold", size = 4)

ggsave(file.path(OUT_DIR, "Figure1B_circos_overview.png"), width = 10, height = 10)
