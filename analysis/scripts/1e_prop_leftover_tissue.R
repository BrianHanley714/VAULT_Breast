...
p = plotting_dataframe%>%
  ...
  geom_boxplot(median.linewidth = 1, whisker.linewidth = 1, staple.linewidth = 1, box.linewidth = 1, outlier.size = 3
               )+
  scale_y_log10(
    labels = scales::label_math(expr = 10^.x, format = log10),
    sec.axis = sec_axis(~ . / 10^8, name = "Tissue Mass (g)", 
                        labels = scales::label_math(expr = 10^.x, format = log10)))+
  scale_fill_manual(values = c("mass" = dark_grey, "numtumourcells" = light_grey))+
  theme_classic(base_size = 40)+
  theme(axis.title.x = element_blank())+
  rot.lab()+
  theme(
    axis.title.y = shadowtext::element_shadowtext(
      angle = 90,
      size = 25,
      colour = light_grey,
      bg.colour = "black",
    ...
    axis.title.y.right = shadowtext::element_shadowtext(
      angle = 90,
      size = 25,
      colour = dark_grey, 
      bg.colour = "black",
...
median(annotations_tumour$numtumourcells_leftover/((annotations_tumour$numtumourcells_FFPEother+
    annotations_tumour$numtumourcells_FFPEmol)))
median(annotations_tumour$numtumourcells_FFPEmol)
median(annotations_tumour$numtumourcells_leftover/annotations_tumour$numtumourcells_50um)

median(annotations_tumour$numtumourcells_leftover/(annotations_tumour$numtumourcells_FFPEmol + annotations_tumour$numtumourcells_FFPEother + annotations_tumour$numtumourcells_leftover))
p+
  stat_compare_means(data = subset(plotting_dataframe, col == "mass"),
                     ...
                     step.increase = 0.06,   # reduce vertical stacking
                     tip.length = 0.01,      # shorten bracket tips
                     vjust = 0.5,
                     bracket.size = 1)+
  theme(
...
pie_df = plotting_dataframe%>%
  ...
  dplyr::filter(col == "mass")%>%
  dplyr::select(Trial_id, sample_type, sample, number_tumourcells)%>%
  dplyr::filter(sample_type %in% c("mass_ERWSI", "mass_FFPEmol", "mass_FFPEother", "mass_leftover"))%>%
  group_by(sample)%>%
